...
fun HomeScreen() {
    ...
    Scaffold(
        ...
        Column(
                ...
                Column(modifier = ComposeModifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                    ...
                    Button(
                        onClick = {
                            scope.launch {
                                busy = true
                                setupMessage = autoSetup.runFullSetup(enable = true).message
                                enabled = true
                                status = checker.check()
                                busy = false
                            }
            ...
            setupMessage?.let { Text(it, color = Success) }
            ...
            if (isLoading) {
                ...
                status?.let { s ->
                    ...
                    StatusCard(if (s.moduleVersion != null) "Module (${s.moduleVersion})" else "Module", s.moduleInstalled)
                    StatusCard("Zygisk .so", s.zygiskLibPresent)
                    StatusCard("Camera1", s.camera1Ready)
                    StatusCard("virtual.mp4", s.hasVirtualVideo)
                    StatusCard("Enabled", s.vcamEnabled)
                    if (s.hookStatus != null) {
                        Text("Hook: ${s.hookStatus} ${s.lastHookPkg ?: ""}")
                    }
                }
            }
        }
...
private fun StatusCard(title: String, ok: Boolean) {
    Card(modifier = ComposeModifier.fillMaxWidth().padding(vertical = 4.dp)) {
        Row(
            modifier = ComposeModifier.fillMaxWidth().padding(12.dp),
            horizontalArrangement = Arrangement.SpaceBetween,
            verticalAlignment = Alignment.CenterVertically
        ) {
            Text(title)
