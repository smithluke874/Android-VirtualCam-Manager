/*
 * VirtualCam Manager — Zygisk native module v1.13.0
 * Pure Magisk (NO LSPosed). Companion gate + JNI Camera hooks + ArtHook ART hooks.
 *
 * v1.13.0: ArtHook integration for Camera.setPreviewTexture / startPreview
 * so virtual.mp4 can replace the live preview surface (classic VCAM algorithm).
 * NV21 PreviewCallback overwrite still next (MediaCodec frame provider).
 */
#include <cstdlib>
#include <cstring>
#include <unistd.h>
...
static void write_module_version() {
    write_text(kModuleVersion, "1.13.0");
    write_text(kVersionFile, "1.13.0");
}
